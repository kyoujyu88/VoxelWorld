...
## 追加調査（2026-07-24）: 高精度スキャンアプリはどう実現しているか

実機で「面が厚い殻になる／別角度で浮きが増える」問題が解けきらないため、先行手法を一次情報で調査した。**結論: 標準は占有カウントではなく TSDF（切断符号付き距離場）フュージョン。** 我々の方式は原理的に不利だった。

### 中核: TSDF フュージョン（KinectFusion 2011 以降の標準）

各ボクセルに **「最寄り面までの符号付き距離」** と **「信頼度の重み」** の 2 値を持つ（[Open3D](https://www.open3d.org/docs/latest/tutorial/t_reconstruction_system/integration.html)）。面の手前が正、面上が 0、奥が負。更新式は加重移動平均（[KinectFusion の詳細記述](https://gist.github.com/savuor/407fdc1807f9d5836d68aebfee726ef7)）:

```
TSDF   = clamp(SDF / truncDist, -1, +1)      # 切断
value  = (W·value_old + w·TSDF) / (W + w)    # 加重平均
W      = min(W + w, maxWeight)               # 重みは上限で飽和
```

これがもたらす性質（＝我々に欠けていたもの）:

1. **サブボクセル精度**: 面は「ゼロ交差」であってセルの中心ではない。ノイズの前後のブレが平均で相殺され、面が真の位置へ収束する。→ 厚い殻ができない。
2. **空間除去が内在的**: 面の手前を通ったレイはそのセルに正の距離を書く。見続ければ平均が正へ押し出され、浮きボクセルは自然消滅する。別のカービング処理は補助でしかない。
3. **重みで距離を扱う**: [Voxblox](https://arxiv.org/pdf/1611.03631) は重みを**センサ距離の 2 乗で減衰**させ、面の裏側で線形に落とすことを推奨。「重み関数の選択が精度に強く影響する」と明記。→ 遠距離観測は自動的に軽くなり、`maxWeight` の飽和と併せて、確立した面は遠くからの一瞥では動かない（我々の場当たり的な「品質ロック」の原理版）。

### 我々の旧方式との対応

| 課題         | 旧（占有カウント）                                 | TSDF                                |
| ------------ | -------------------------------------------------- | ----------------------------------- |
| 面が厚い     | セル単位でしか表現できず前後のブレが別セルに積もる | ゼロ交差がサブボクセルで収束        |
| 浮きボクセル | 別パスで削る（遅い・調整難）                       | 手前を通るたび正へ押され消える      |
| 遠距離の劣化 | 品質ロックを後付け                                 | 重み 1/z² ＋ 重み飽和で原理的に解決 |
| しきい値     | 無制限に増える count（Phase 6.4 で判明した不備）   | 重みは maxWeight で飽和＝有界       |

### 併せて確認した周辺技術（今回は未採用・将来の選択肢）

- **ポーズのドリフト対策**: [RTAB-Map](http://introlab.github.io/rtabmap/) は bag-of-words によるループ閉じ込み検出とポーズグラフ最適化を**オンラインで**行い、蓄積誤差を吸収する（iOS 版は ARKit の VIO を前段に使う）。我々は ARCore のポーズをそのまま信頼しており、広域スキャンではドリフトが残る。
- **深度前処理**: RGB-D では物体の輪郭で「flying pixels（宙に浮く画素）」が出るため、近傍距離フィルタやバイラテラルフィルタで除去するのが定石（[例](https://arxiv.org/pdf/2410.08084)）。深度勾配の大きい画素を捨てるだけでも効果がある。
- **面の法線角度**: 視線と面が浅い角度（grazing）の観測は誤差が大きく、重みを下げる/捨てるのが一般的（voxblox の anti-grazing）。

### 反映方針（Phase 7 で実装）

- グリッドを **sdf + weight** に置き換え、面は **|sdf| ≤ surfaceBand** のセルとして抽出。
- 各深度点は「1 セル」ではなく**視線に沿った帯**（手前 +trunc 〜 奥 -trunc/2）に符号付き距離を書き込む。
- 重みは **1/z²**、`maxWeight` で飽和。色は面の近傍のみ、重み付き平均。
- 未採用（次の候補）: 深度エッジ除去、法線角度による重み付け、ループ閉じ込み。
...
## 平面検出（plane detection）— 調査 2026-07-26

「壁や床と認識できたら決め打ちにして高速化・軽量化できないか」という問いに対する一次情報の確認。

### API（一次情報: immersive-web/real-world-geometry の explainer）

- フィーチャ記述子は **`"plane-detection"`**。`requestSession` の `requiredFeatures` / `optionalFeatures` に入れる
- 有効なら各 `XRFrame` に **`detectedPlanes`**（`XRPlane` の集合）が生える。**完全に同期 API**
- `XRPlane` の内容:
  - `planeSpace` — 平面中心の `XRSpace`。**Y 軸が平面の法線**、X/Z が right/top
  - `polygon` — 平面の形を近似する**凸多角形**の頂点列（`planeSpace` 座標系、Y は必ず 0）
  - `orientation` — `"horizontal"` / `"vertical"`（分類できないと未定義）
  - `lastChangedTime` — `rAF` の `timestamp` と比較して、そのフレームで更新されたか判定できる
- **同一の平面は同一のオブジェクト**で返る（フレームを跨いで `Set` 比較で追加/削除を検出できる）
- 属性が有効なのは **`requestAnimationFrame` コールバック中だけ**。前フレームの情報が要るならコピーが必要
- 追跡を失った平面は集合から消え、以後**プロパティアクセスは例外を投げる**

### 注意点（explainer が明記しているもの）

- **平面は時間とともに精緻化される**。ポーズも polygon も変わる前提で書く必要がある
- **subsume（吸収）が起きる**。理解が進むと複数の平面が 1 枚に統合され、吸収された側は削除扱いになる
- 機能が無効なときの `detectedPlanes` アクセスが**例外**で、graceful な検出手段がない（spec issue #30）→ 参照は必ず try/catch で包む

### 実機での可用性

Chrome では長く **Developer Trial** 扱いだった（chromestatus の Intent to Ship では 147 で既定有効の予定と読める）。**推測せず端末に答えさせる**方針を取り、`plane-detection` を **optionalFeatures に追加**して HUD に `平面検出` 行（可否と検出枚数、水平/垂直の内訳）を出すようにした。`optionalFeatures` なので非対応端末でもセッションは従来どおり開始する。

### 期待できる効果（実測値からの見積もり）

Phase 8c 後の実機は CPU 計 29.0ms のうち **`融合` 15.5ms が最大項目**。融合コストは深度サンプル数（1,620/frame）に比例し、1 サンプルあたり帯 4 セル分のハッシュ参照と加重平均を行っている。

平面と分かった領域では、(a) 帯の書き込みを省いて平面パラメータの更新だけにできる、(b) そもそも TSDF セルとして保持しない、の 2 つが効く。部屋のスキャンでサンプルの過半が壁/床/天井に当たると仮定すると、`融合` と `除去`（描画インスタンス数に比例）の双方が下がる。

さらに**精度面の効果が大きい**: 平面フィットは数千の測定を 4 パラメータに集約するのでノイズが 1/√N で落ちる。実機スクリーンショットで見えていた「平らな壁がボコボコ」「床の上に浮きボクセルが散る」は、まさに平面スナップが直す症状。

### 未解決の設計課題

- **境界**: 壁がどこで終わるか（ドア、窓、幅木）。ARCore の polygon は凸包なので実際の開口を表現できない
- **壁際の物体**を壁に吸収してしまわないための分離規則
- 出力はあくまでボクセル（マイクラ的な見た目）なので、平面は**内部表現**に留め、表示/エクスポート時にボクセル化し直す必要がある。逆に言えば平面をラスタライズすると完全に平らなボクセル板になるので、見た目としてはむしろ望ましい
- ARCore の平面が使えない端末向けに、自前の平面抽出（RANSAC / 法線の領域成長）が必要になる可能性
