...
### 方法 A（最推奨・開発機不要）: GitHub Pages（HTTPS）

`main` に push すると GitHub Actions（`.github/workflows/deploy-pages.yml`）が自動でビルド＆デプロイし、HTTPS で公開する。**開発機も adb も不要** — Pixel の Chrome で URL を開くだけ。WebXR は secure context 必須だが、GitHub Pages は HTTPS なので条件を満たす。

- 公開 URL: **`https://kyoujyu88.github.io/VoxelWorld/`**（末尾スラッシュ必須）
- 初回のみ設定が要る場合がある: リポジトリ **Settings → Pages → Build and deployment → Source を「GitHub Actions」**に。ワークフローが `enablement: true` で自動有効化を試みるが、権限次第で手動が必要なことがある。
- デプロイ状況・失敗ログは **Actions タブ → 「Deploy to GitHub Pages」** で確認。
- サブパス配信のため `VITE_BASE=/VoxelWorld/` でビルドしている（アセットは `/VoxelWorld/assets/...` に解決）。**リポジトリ名を変えたら base も変える**こと。
- ⚠️ **github.com のリポジトリ画面**や、末尾スラッシュ無しの直リンクではアプリは動かない（＝「何も表示されない」の原因）。必ず上記 Pages URL を開くこと。
...
### 方法 B（開発機あり）: `adb reverse` で localhost 転送

証明書不要。開発機に Node と Android SDK Platform-Tools（adb）が必要。ホットリロードで開発しながら実機確認したいときに便利。
