...
| 1 | 環境確認ページ | ⬜ | Pixel 9a Chrome で `depth-sensing` 利用可能と表示 | 未着手（計画承認待ち） |
...
1. **計画承認待ち**: Phase 1 着手前に実装計画の確認を依頼中（プロンプト Section 10-4）。
2. **色の方針（Phase 3）**: camera-access で実色を取るか、幾何優先で色を後回しにするか。→ 要判断。
